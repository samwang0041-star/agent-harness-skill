...
[ ] For long-job delivery in shell-capable runtimes, start "$HARNESS_SKILL_DIR/scripts/harness-monitor.sh" from this same agent session, or record why it was skipped.
...
[ ] For Claude Code-style work, confirm the active session started "$HARNESS_SKILL_DIR/scripts/harness-monitor.sh" against the same handoff workspace, or documented why not.
